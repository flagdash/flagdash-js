...
function createMockClient(overrides: Record<string, any> = {}) {
  ...
  return {
    ...
    experiment: vi.fn().mockResolvedValue(null),
    trackExperimentMetric: vi.fn(),
...
describe('useExperiment', () => {
  it('resolves an assignment once for an equivalent inline context', async () => {
    const assignment = {
      key: 'checkout-flow',
      status: 'running',
    ...
    const client = createMockClient({
      experiment: vi.fn().mockResolvedValue(assignment),
    });
    const wrapper = createWrapper(client);

    const { result, rerender } = renderHook(
      () => useExperiment('checkout-flow', { user: { id: 'user-123' } }),
      { wrapper }
    );

    await waitFor(() => expect(result.current.assignment).toEqual(assignment));
    rerender();
    expect(client.experiment).toHaveBeenCalledTimes(1);
    expect(result.current.isLoading).toBe(false);
  });
});

describe('useExperimentMetric', () => {
  it('queues the event through the shared client', () => {
    const client = createMockClient();
    const { result } = renderHook(() => useExperimentMetric(), { wrapper: createWrapper(client) });
    const event = { experimentKey: 'checkout-flow', eventName: 'checkout_completed', context: { user: { id: 'alice' } } };
    act(() => result.current(event));
    expect(client.trackExperimentMetric).toHaveBeenCalledWith(event);
  });
});
...
describe('useAiConfig', () => {
  ...
  it('uses default content when provided', () => {
    ...
    // Before resolution, falls back to defaultContent
