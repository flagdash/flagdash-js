...
import { ReactNativeClient, hashKey } from "../client";
...
describe("ReactNativeClient", () => {
  ...
  it("loadCache() returns cached data", async () => {
    // The cache namespace is a hash of the sdkKey, not a slice of it, so the
    // key material never lands in an AsyncStorage key name.
    const sdkKey = "sk_abc12345_rest_of_key";
    const prefix = hashKey(sdkKey);
  ...
  it("cache key names never contain key material", async () => {
    // This is the reason the namespace is hashed: AsyncStorage key names are
    // stored in cleartext on the device, so a raw slice of the sdkKey would
    // persist a fragment of a secret.
    const sdkKey = "sk_supersecretkeymaterial";
    const prefix = hashKey(sdkKey);

    expect(prefix).not.toContain("supersecret");
    // No four-character-or-longer run of the key survives into the namespace.
    for (let i = 0; i + 4 <= sdkKey.length; i++) {
      expect(prefix).not.toContain(sdkKey.slice(i, i + 4));
    }
  });
