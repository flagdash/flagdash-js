...
import type { FlagValues, ConfigValues, EvaluationContext, FlagDetail, AiConfig, ListAiConfigsOptions, EventListener, FlagDashEvent, TranslationOptions, TranslationCatalog } from "@flagdashio/sdk";
...
export class ReactNativeClient {
  /** Evaluate a release for a stable identity without resolving credentials. */
  aiConfigRelease(key: string, userId?: string) {
    return this.client.aiConfigRelease(key, userId);
  }
  ...
  constructor(config: ReactNativeConfig) {
    ...
    // Derive the cache namespace from a hash of the sdkKey rather than a raw
    // slice of it. Storing key material (even a prefix) in cleartext
    // AsyncStorage key names needlessly persists a secret fragment on device.
    this.cachePrefix = hashKey(config.sdkKey);
  ...
  translation(key: string, options: TranslationOptions): Promise<string> {
    return this.client.translation(key, options);
  }

  translations(locale: string, namespace: string): Promise<TranslationCatalog> {
    return this.client.translations(locale, namespace);
  }
...
/**
 * Derive a short, stable, non-reversible namespace from the sdkKey for use in
 * AsyncStorage cache key names. Uses a djb2-style hash (no crypto dependency);
 * collision resistance is not required — it only needs to be stable per key and
 * not leak key material.
 */
export function hashKey(sdkKey: string): string {
  let hash = 5381;
  for (let i = 0; i < sdkKey.length; i++) {
    hash = ((hash << 5) + hash + sdkKey.charCodeAt(i)) >>> 0;
  }
