import type { FlagValues, ConfigValues } from "@flagdashio/sdk";

// SECURITY NOTE: flag and config values are persisted here as plaintext JSON.
// The default backend (@react-native-async-storage/async-storage) is NOT
// encrypted — its contents are readable on rooted/jailbroken devices and can
// be extracted from unencrypted device backups. This is fine for public
// flag/config values, but do NOT rely on cached values to gate access to
// sensitive data on the client. If your values are sensitive, inject an
// encrypted backend (e.g. one backed by expo-secure-store / react-native-keychain
// / MMKV with encryption) via `setAsyncStorage(...)` — any implementation of
// `AsyncStorageLike` is accepted.
