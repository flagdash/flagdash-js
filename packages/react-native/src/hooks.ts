...
import type { EvaluationContext, AiConfig, ListAiConfigsOptions, FlagDetail, TranslationCatalog, TranslationOptions } from "@flagdashio/sdk";
import { FlagDashContext } from "./context";
import type { ReactNativeClient } from "./client";

function useFlagDashClient(): { client: ReactNativeClient; isReady: boolean } {
...
export function useFlag<T = boolean>(
  ...
  // Stabilize `context`/`defaultValue`: inline object/array literals are a new
  // reference every render, so putting them in the deps re-runs the effect
  // (re-subscribe + re-fetch, context path uncached) each render. Depend on a
  // serialized context key and read latest values through refs.
