...
describe('FlagDashServerClient', () => {
  it('throws if sdkKey is missing', () => {
    expect(() => new FlagDashServerClient({ sdkKey: '', environment: 'test' })).toThrow(
  ...
  it('throws if environment is missing', () => {
    expect(
      () => new FlagDashServerClient({ sdkKey: 'server_sk_123', environment: '' })
    ).toThrow('FlagDash: environment is required');
  });
