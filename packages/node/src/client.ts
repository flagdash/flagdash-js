...
export class FlagDashServerClient {
   ...
   * ```
   */
  /** Explicit backend fetch. Requires configs:read and secrets:read; never cached or defaulted. */
  async resolveConfig(key: string): Promise<Config> {
    const response = await this.request<{config: Config}>(`/server/configs/${encodeURIComponent(key)}/resolve`,
      {method: "POST", body: "{}", cache: "no-store"});
    if (!response.config) throw new Error("FlagDash: resolved config missing");
    return response.config;
  }

  /** Evaluate an AI Release and resolve only its credentials. Never cached or defaulted. */
  /** Evaluate a release without resolving secrets. Decisions are never cached. */
  async aiConfigRelease(key: string, userId?: string): Promise<import("./types").AiConfigReleaseResult | null> {
    try {
      const query = userId ? `?user_id=${encodeURIComponent(userId)}` : "";
      const response = await this.request<{ai_config: import("./types").AiConfigReleaseResult}>(
        `/ai-config-releases/${encodeURIComponent(key)}${query}`
      );
      return response.ai_config;
    } catch { return null; }
  }

  async resolveAiRelease(key: string, userId = "anonymous"): Promise<{key: string; version: number; config: Record<string, unknown>; reason: string; variation_key: string | null; rollout_percentage: number}> {
    const response = await this.request<{ai_config: {key: string; version: number; config: Record<string, unknown>; reason: string; variation_key: string | null; rollout_percentage: number}}>(
      `/server/ai-config-releases/${encodeURIComponent(key)}/resolve`,
      {method: "POST", body: JSON.stringify({user_id: userId}), cache: "no-store"});
    if (!response.ai_config) throw new Error("FlagDash: resolved AI release missing");
    return response.ai_config;
  }
