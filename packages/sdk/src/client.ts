...
export class FlagDashClient {
  private opts: Required<
    Pick<FlagDashConfig, "sdkKey" | "environment" | "baseUrl" | "timeout">
  > &
    Pick<FlagDashConfig, "refreshInterval">;
  ...
  constructor(config: FlagDashConfig) {
    ...
    if (!config.environment) throw new Error("FlagDash: environment is required");
    ...
    this.refreshFlags()
      .then(() => {
        ...
        if (this.opts.refreshInterval && this.opts.refreshInterval > 0) {
          this.startPolling();
        }
      })
      .catch(() => {
        // Error already emitted in refreshFlags
      });
  }
  ...
  /**
   * Stop background polling. Call this when you no longer need live updates.
   */
  destroy() {
