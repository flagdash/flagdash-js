...
describe('partial permissions', () => {
  // A key scoped to flags but not configs must still deliver flags. Before this
  // the constructor used Promise.all, so a 403 on /configs rejected the chain,
  // `ready` never fired, and every consumer hung on a resource it never asked
  // for.
  const forbidConfigs = () =>
    createFetchMock({
      '/configs': () =>
  ...
  it('becomes ready and serves flags when /configs is forbidden', async () => {
    mockFetch.mockImplementation(forbidConfigs());

    const client = await createReady();

    expect(await client.flag('test-flag')).toBe(true);
    expect(await client.flag('other')).toBe('hello');
  });

  it('falls back to the default for configs it cannot read', async () => {
    mockFetch.mockImplementation(forbidConfigs());

    const client = await createReady();

    expect(await client.config('pricing', 'fallback')).toBe('fallback');
  });

  it('still becomes ready when both resources are forbidden', async () => {
    ...
    // Hanging forever is worse than serving defaults — callers get their
    // fallback value instead of a spinner that never resolves.
    const client = await createReady();

    expect(await client.flag('test-flag', undefined, false)).toBe(false);
  });

  it('reports which resource was refused', async () => {
    mockFetch.mockImplementation(forbidConfigs());
    ...
    const refused = errors.find((e) => e?.status === 403);
    expect(refused).toBeDefined();
    expect(refused.path).toBe('/configs');
    expect(refused.message).toContain('/configs');
  });
});
...
describe('FlagDashClient', () => {
  ...
  it('experiment() returns the normalized stable assignment', async () => {
    mockFetch.mockImplementation(createFetchMock({
      '/experiments/checkout-flow': () => Promise.resolve({
        ok: true,
        json: () => Promise.resolve({
          experiment: {
            ...
            parameters: { checkout_layout: 'compact' },
            reason: 'experiment',
          },
        }),
      }),
    ...
    const client = await createReady();
    const assignment = await client.experiment('checkout-flow', { user: { id: 'alice' } });
    ...
    const call = mockFetch.mock.calls.find(([url]: [string]) => url.includes('/experiments/checkout-flow'));
    expect(call?.[0]).toContain('user_id=alice');
    client.destroy();
  });

  it('experiment() refuses anonymous assignment without making a request', async () => {
    const client = await createReady();
    const callsBefore = mockFetch.mock.calls.length;

    expect(await client.experiment('checkout-flow', {})).toBeNull();
    expect(mockFetch.mock.calls.length).toBe(callsBefore);
    client.destroy();
  });

  it('buffers and flushes experiment metrics with stable identity', async () => {
    mockFetch.mockImplementation(createFetchMock({
      '/experiment-events/batch': () => Promise.resolve({ ok: true, json: () => Promise.resolve({ accepted: 1 }) }),
    }));
    const client = await createReady();
    client.trackExperimentMetric({ experimentKey: 'checkout-flow', eventName: 'checkout_completed', context: { user: { id: 'alice' } }, eventId: 'evt-fixed' });
    await client.flushExperimentEvents();
    const call = mockFetch.mock.calls.find(([url]: [string]) => url.includes('/experiment-events/batch'));
    expect(JSON.parse(call?.[1].body).events[0]).toMatchObject({ event_id: 'evt-fixed', experiment_key: 'checkout-flow', event_name: 'checkout_completed', user_id: 'alice' });
    client.destroy();
  });

  it('config() returns the fallback when initial config loading is forbidden', async () => {
    mockFetch.mockImplementation(createFetchMock({
      '/configs': () => Promise.resolve({ ok: false, status: 403, statusText: 'Forbidden' }),
    }));
    const client = new FlagDashClient({ sdkKey: 'sk_flags_only', baseUrl: 'http://localhost:4000' });
    try { expect(await client.config('pricing', 'fallback')).toBe('fallback'); }
...
describe('FlagDashClient — SSE (real-time)', () => {
  ...
  it('retries SSE with exponential backoff (with jitter) on error', async () => {
    vi.useFakeTimers();
    MockEventSource.reset();
    ...
    // Backoff for retry 1 is base 1000ms with full jitter → [500ms, 1500ms].
    // Advancing by the upper bound guarantees the reconnect timer has fired
    // regardless of the random jitter value.
    ...
    // Retry 2 backoff base is 2000ms → jittered upper bound 3000ms.
    await vi.advanceTimersByTimeAsync(3000);
